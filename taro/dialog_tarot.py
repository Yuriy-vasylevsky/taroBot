...
# # BACKGROUND_PATH = str(BASE_DIR / "background.png")
# # BACKGROUND_PATH10 = str(BASE_DIR / "bg.png")
# # EXIT_TEXT = "⬅️ Завершити бесіду"
# # WELCOME_IMAGE = str(BASE_DIR / "assets" / "1.png")
...
# # ================== RATE LIMIT + TEMP CLEANUP ==================
# def is_rate_limited(user_id: int) -> bool:
#     now = time.monotonic()
#     last = LAST_MESSAGE_TS.get(user_id, 0)
...
#     """Видаляє всі тимчасові файли старше 10 хвилин"""
#     try:
#         for f in BASE_DIR.glob("temp_*.png"):
...
#     logger.error(f"OpenAI failed after {OPENAI_RETRIES+1} attempts", extra={"user_id": user_id})
...
#         ]
#     if amount == 4:
#         return "Стосунки (4)", ["Як виглядає зв’язок загалом", "Почуття/намір між вами", "Що напружує / що заважає", "Куди це рухається (вектор)"]
#     if amount == 5:
#         return "Поглиблений розклад ситуації (5)", ["Поточна ситуація", "Ресурс / що допомагає", "Виклик / що заважає", "Приховане / те, чого не видно", "Ймовірний напрямок / результат"]
...
# class SpinnerHandle:
#     def __init__(self, anim_msg: types.Message, text_msg: types.Message, stop_event: asyncio.Event, task: asyncio.Task):
...
#         await self._safe_delete(self.text_msg, "text")
#         await asyncio.sleep(0.4)
#         await self._safe_delete(self.anim_msg, "animation")

#     async def _safe_delete(self, msg: types.Message, msg_type: str):
...
#     if (is_likely_question(original_text) or has_topic_markers(original_text)) and len(original_text) > 8 and not is_smalltalk_question(original_text):
#         amount = rule_based_amount(user_text) or 3
...
#         if (is_likely_question(user_text) or has_topic_markers(user_text) or "про " in normalize_text(user_text)) and len(user_text) > 8:
...
#                     payload = f"ПОПЕРЕДНІЙ КОНТЕКСТ:\n{prev_summary}\n\nВитягнута карта:\n1. {clar_card['ua']} ({clar_card['code']}) {arrow}"

#                     spinner = await start_spinner(message)

#                     resp = await _openai_create_with_retry(
#                         model="gpt-4o-mini",
#                         messages=[{"role": "system", "content": CLARIFIER_PROMPT}, {"role": "user", "content": payload}],
#                         max_tokens=1600,
#                         temperature=0.82,
#                         user_id=user_id,
#                     )
...
#                     messages=[{"role": "system", "content": TAROT_SYSTEM_PROMPT}, {"role": "user", "content": payload}],
#                     max_tokens=2000,
#                     temperature=0.82,
#                     user_id=user_id,
#                 )
...
#                 await spinner.stop()
#             cleanup_temp_files()
#             if "final_img_path" in locals() and os.path.exists(final_img_path):
#                 try:
...
# print("✅ Tarot dialog module loaded (виправлено: dialog_router + client визначені, продакшен-готовий)")


# клод версія, готова в прод
import os
import re
import json
import random
