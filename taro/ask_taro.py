...
#     card_h = int(cards[0].size[1] * ratio * 1.05)  # реальна висота з тінню
...
#         # Обрізаємо зображення якщо воно виходить за межі фону
#         paste_x = max(0, x)
#         paste_y = max(0, y)
...
#     await message.answer("🔍 [DEBUG] WebApp дані отримано, починаю обробку...")
    
#     try:
#         data = json.loads(message.web_app_data.data)
#     except Exception as e:
#         await message.answer(f"❌ [DEBUG] Помилка парсингу JSON: {e}")
#         return

#     await message.answer(f"🔍 [DEBUG] action={data.get('action')}, карт={len(data.get('chosen', []))}")

#     if data.get("action") != "three_cards":
#         await message.answer(f"❌ [DEBUG] Невірний action: {data.get('action')}")
#         return

#     chosen = data.get("chosen", [])
#     if len(chosen) != 3:
#         await message.answer(f"❌ [DEBUG] Кількість карт: {len(chosen)}, потрібно 3")
#         return
...
#     await message.answer(f"🔍 [DEBUG] question={bool(question)}, layout={bool(layout)}")

#     if not question or not layout:
#         await message.answer("❌ [DEBUG] Немає question або layout в стані. Спробуй заново.")
#         await state.clear()
#         return
...
#         if not info:
#             await message.answer(f"❌ [DEBUG] Карта не знайдена в TAROT_CARDS: '{eng}'")
#             continue

#         img_path = info["image"]
#         import os as _os
#         exists = _os.path.exists(img_path)
#         await message.answer(f"🔍 [DEBUG] Карта {i}: {eng} → {img_path} | існує: {exists}")
...
#     if len(img_paths) != 3:
#         await message.answer(f"❌ [DEBUG] Завантажено {len(img_paths)} з 3 карт.")
#         await state.clear()
#         return
...
#         # Якщо зображення не вдалось — показуємо карти текстом і продовжуємо
...
#             i += 1
#             try:
#                 await asyncio.wait_for(stop_anim.wait(), timeout=0.25)
